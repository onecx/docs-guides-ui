...
Since version 6 of OneCX `@onecx/portal-integration-angular` library is no longer available. The list of features that are still supported in different libraries or solutions can be found in the <<removed-from-integration-angular, removed from @onecx/portal-integration-interface>> section. It is important to <<replace-portal-core-module, replace PortalCoreModule>> if used in the application to preserve existing functionality.
...
The above snippet assumes that the exposed style sheet is in *src/styles.scss*. Setting the *inject* property to *true* might lead to hashing meaning the exposed file would be called *styles.HASH.css*, where *HASH* is generated hash value during project building. If this applies to your application, make sure to adjust the exposal so hash is not included.

[#replace-portal-core-module]
=== Replace PortalCoreModule
If your application was using *PortalCoreModule* from *@onecx/portal-integration-angular*, it should replace all occurrences with *AngularAcceleratorModule* from *@onecx/angular-accelerator*. This will make sure that all components and directives 
...
=> *Please, consider using the InteractiveDataViewComponent (ocx-interactive-data-view) from @onecx/angular-accelerator*

* PageContentComponent (ocx-page-content) +
=> *Please, consider using the OcxContentComponent (ocx-content) or OcxContentContainerComponent (ocx-content-container) from @onecx/angular-accelerator*
