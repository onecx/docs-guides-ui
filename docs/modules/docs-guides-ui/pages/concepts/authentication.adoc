...
WARNING: Use `AUTH_SERVICE=disabled` only for local development and testing, such as running the shell without a Keycloak instance available. Never use it in a production or otherwise publicly reachable environment — it removes authentication entirely, leaving every application and backend call unprotected.
...
* xref:documentation:docs-guides-ui:features/permissions.adoc[Permissions Handling] — rendering UI conditionally based on the permissions the shell exposes for the current user.
