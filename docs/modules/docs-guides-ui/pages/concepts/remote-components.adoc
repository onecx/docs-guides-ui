...
Most of what a user sees in OneCX comes from *modules*: an application registers one or more routes with the shell via xref:documentation:docs-guides-ui:module-federation/index.adoc[module federation], and the shell loads and renders that application's module whenever the user navigates to one of its routes. A module owns a whole page (or a whole section of the URL space) and is only ever loaded by the shell itself.
...
* xref:documentation:docs-guides-ui:module-federation/index.adoc[Module Federation in OneCX] — the underlying mechanism Remote Components are loaded through, and how it differs from loading a module.
