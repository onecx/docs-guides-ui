...
Most of what a user sees in OneCX comes from *modules*: an application's routes and microfrontends are configured via OneCX platform configuration, and the shell loads and renders that application's module via xref:documentation:docs-guides-ui:module-federation/index.adoc[module federation] whenever the user navigates to one of its routes. A module owns a whole page (or a whole section of the URL space) and is only ever loaded by the shell itself.
