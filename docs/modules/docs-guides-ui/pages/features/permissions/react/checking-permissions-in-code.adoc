...
[NOTE]
====
This API is scoped by `appId` and `productName` and resolves permissions over the `PermissionsRpcTopic` — the remote-components data path. It is a different source from the Angular `UserService` / `PermissionService` standard path documented on the {checking_permissions_code}[Checking permissions in code (Angular)] page. Both reflect the same underlying role-based assignments, but they read them through different integrations.
====
...
* {checking_permissions_code}[Checking permissions in code (Angular)] — the Angular `PermissionService` / `HAS_PERMISSION_CHECKER` equivalent.
