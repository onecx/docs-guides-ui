...
[NOTE]
====
The directives require the {user_service}[User Service] to be provided for the current user's permissions to be resolved.
====
...
* {checking_permissions_code}[Checking permissions in code] — the `UserService` checks for logic outside a template.
