...
[#setup]
== Setup
To setup the multi-language translations support follow the steps listed below.
...
Optionally, if you intend to support multiple locales at once in case of missing translations for the current locale, you can utilize the `MultiLanguageMissingTranslationHandler` available via `@onecx/angular-utils`:
