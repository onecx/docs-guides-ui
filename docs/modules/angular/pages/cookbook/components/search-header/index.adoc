...
[#ocx-advanced-directive]
=== ocxAdvanced Directive

The `*ocxAdvanced` directive is a structural directive that provides advanced search field visibility control within the SearchHeader component. This directive enables the creation of a two-tier search interface where fields can be categorized as either basic or advanced.

* **Basic Mode**: Fields marked with `*ocxAdvanced` are hidden by default
* **Advanced Mode**: All fields (basic and advanced) are visible
* **Toggle Button**: The Basic|Advanced toggle button appears automatically when at least one field uses the `*ocxAdvanced` directive
